var feature1_ ,feature2_, feature3_, prediction1, prediction2, prediction3, prediction_pr1, prediction_pr2 , prediction_pr3;
...
$(document).on('click','#submit',function(){
    ...
    if(feature1 == "" || feature2 == "" || feature3 == ""){
      ...
      alert("Прошу вас заполнить все поля");
    }
    else{
       
      // replace <username> with your pythonanywhere username
      // also make sure to make changes in the url as per your flask API argument names
      var requestURL = "https://Katalip.pythonanywhere.com/predict/?text="+feature1+"&text="+feature2+"&text="+feature3;
      console.log(requestURL); // log the requestURL for troubleshooting
      $.getJSON(requestURL, function(data) {
        console.log(data); // log the data for troubleshooting
        prediction1 = data['pred'][0];
        prediction2 = data['pred'][1];
        prediction3 = data['pred'][2];
          
        prediction_pr = data['pred_probs'];  
      });
      ...
      $(".result1").html("Пост является: " + prediction1);
      $(".result2").html("Пост является: " + prediction2);
      $(".result3").html("Пост является: " + prediction3);
      $(".result4").html("Вероятности: " + prediction_pr);
    }
  });